...
class ConvertTest < Minitest::Test
  ...
  def test_latlng
    assert_equal("1.000000,2.000000", GoogleMaps::Convert.latlng({"lat": 1, "lng": 2}))
    assert_equal("1.000000,2.000000", GoogleMaps::Convert.latlng([1, 2]))

    assert_raises(ArgumentError) { GoogleMaps::Convert.latlng(1) }
    assert_raises(ArgumentError) { GoogleMaps::Convert.latlng("test") }
  end

  def test_join_list
    assert_equal("asdf", GoogleMaps::Convert.join_list("|", "asdf"))
    assert_equal("1,2,A", GoogleMaps::Convert.join_list(",", ["1", "2", "A"]))
    assert_equal("", GoogleMaps::Convert.join_list(",", []))
  end

  def test_as_list
    assert_equal([1], GoogleMaps::Convert.as_list(1))
    assert_equal([1, 2, 3], GoogleMaps::Convert.as_list([1, 2, 3]))
    assert_equal(["string"], GoogleMaps::Convert.as_list("string"))

    a_dict = {"a": 1}
    assert_equal([a_dict], GoogleMaps::Convert.as_list(a_dict))
  end

  def test_time
    assert_equal("1409810596", GoogleMaps::Convert.time(1409810596))

    t = Time.at(1409810596)
    assert_equal("1409810596", GoogleMaps::Convert.time(t))

    dt = Time.at(1409810596).to_datetime
    assert_equal("1409810596", GoogleMaps::Convert.time(dt))
  end

  def test_components
    c = {"country": "US"}
    assert_equal("country:US", GoogleMaps::Convert.components(c))

    c = {"country": "US", "foo": 1}
    assert_equal("country:US|foo:1", GoogleMaps::Convert.components(c))

    assert_raises(ArgumentError) { GoogleMaps::Convert.components("test") }
    assert_raises(ArgumentError) { GoogleMaps::Convert.components(1) }
  end
  ...
  def test_bounds
    ...
    assert_equal("3.000000,4.000000|1.000000,2.000000", GoogleMaps::Convert.bounds(b))

    assert_raises(ArgumentError) { GoogleMaps::Convert.bounds("test") }
  end
  ...
  def test_polyline_decode
    ...
    points = GoogleMaps::Convert.decode_polyline(syd_mel_route)
